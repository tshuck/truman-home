...
class WeConfessed extends Draw {
  ...
  periodToTalking() {
    const from = this.right(this.period)
    const to = this.top(this.talkingAbout)
  ...
  theToName() {
    const from = this.right(this.the)
    const to = this.left(this.nameWas)
  ...
  nameToHell() {
    const from = this.right(this.nameWas)
    const to = this.left(this.hell)
  ...
  hellToAndWe() {
    const from = this.right(this.hell)
    const to = this.bottom(this.andWeWere)
  ...
  becauseToWe() {
    const from = this.bottom(this.because)
    const to = this.bottom(this.weDoubted)
  ...
  weToTheOther() {
    const from = this.right(this.weDoubted)
    const to = this.bottom(this.theOther)
  ...
  sToItWas() {
    const from = this.right(this.s)
    const to = this.top(this.itWasStrange)
  ...
  usedToToPicture() {
    const from = this.right(this.usedTo)
    const to = this.left(this.pictureHim)
  ...
  colon2ToEverything() {
    const from = this.right(this.colon2)
    const to = this.bottom(this.everythingIs)
  ...
  sinsToOr() {
    const from = this.bottom(this.sinsFestering)
    const to = this.left(this.or)
  ...
  orToAFaint() {
    const from = this.right(this.or)
    const to = this.left(this.aFaintWhisper)
  ...
  pastToWhen() {
    const from = this.right(this.past)
    const to = this.left(this.whenYoure)
  ...
  whiteToBut() {
    const from = this.right(this.pureDazzling)
    const to = this.left(this.but)
  ...
  stinkToSave() {
    const from = this.right(this.stinkYou)
    const to = this.left(this.saveYourself)
    ...
    return <PoemsLayout>
      <SVGContainer ref={this.svg}>
        <Page bg="#CD5C5C">
          ...
          <Line>
            <Word ref={this.science}>science</Word> for the next sacrament<Word ref={this.colon}>:</Word> Confirmation. <Word ref={this.the}>The</Word> priest said Exam-
          </Line>
          <Line>
            ination of Conscience and confession would save us from <Word ref={this.hell}>hell</Word>. His
          </Line>
          <Line>
            <Word ref={this.nameWas}>name was</Word> Father White <Word ref={this.andWeWere}>and we were interested</Word> in him <Word ref={this.because}>because</Word> one
          </Line>
          <Line>
          ...
          <Line>
            drunk, <Word ref={this.becauseEveryone}>because everyone</Word> knows they’re not supposed to. I <Word ref={this.usedTo}>used to</Word>
          </Line>
          <Line>
            look at buses passing by and <Word ref={this.pictureHim}>picture him</Word> up there, <Word ref={this.smilingAway}>smiling away</Word> and
          </Line>
          <Line>
          ...
          <Line>
            <Word ref={this.everythingIs}>Everything is either</Word> a sin <Word ref={this.or}>or</Word> not a sin and that’s an idea you might
          </Line>
          <Line>
            carry in your head the rest of your life. Then when you grow up and
          </Line>
          <Line>
            drift away from the church, Mea culpa is <Word ref={this.aFaintWhisper}>a faint whisper</Word> in your <Word ref={this.past}>past.</Word>
          </Line>
          <Line>
            It’s still there, but now you’re older and not so easily frightened.
          </Line>
          <Line indent={true}>
            <Word ref={this.whenYoure}>When you’re</Word> in a state of grace the soul is a <Word ref={this.pureDazzling}>pure dazzling white</Word>
          </Line>
          <Line>
            surface, <Word ref={this.but}>but</Word> your sins create abscesses that ooze and <Word ref={this.stinkYou}>stink. You try to</Word>
          </Line>
          <Line>
            <Word ref={this.saveYourself}>save yourself with</Word> Mea culpa, <Word ref={this.theOnly}>the only</Word> Latin <Word ref={this.wordsThatMean}>words that mean any-</Word>
          </Line>
          <Line>
