...
class YourePrimal extends Draw {
  ...
  private youngToYouKnowThat() {
    const from = Draw.left(this.young)
    const to = Draw.top(this.youKnowThat)
  ...
  private couldToBe() {
    const from = Draw.bottom(this.could)
    const to = Draw.bottom(this.be)
  ...
  private knewToYourself() {
    const from = Draw.bottom(this.knew2)
    const to = Draw.bottom(this.yourself)
  ...
  private firstToStand() {
    const from = Draw.right(this.first)
    const to = Draw.right(this.stand)
  ...
  private playToFearless() {
    const from = Draw.right(this.play)
    const to = Draw.right(this.fearless)
    ...
    return (
      <SVGContainer ref={this.svg}>
        <Page bg="#cb4b16">
          ...
          <Line>must have decided where you’ll be — “posture and placement” — and</Line>
          <Line>
            who you’ll be — “<Word ref={this.identity}>identity</Word> and <Word ref={this.image}>image</Word>.” I never{' '}
            <Word ref={this.knew2}>knew</Word> teaching <Word ref={this.could}>could</Word>
          </Line>
          <Line>
            <Word ref={this.be}>be</Word> that <Word ref={this.complicated}>complicated</Word>. He said, You{' '}
            <Word ref={this.simply}>simply</Word> cannot teach unless you
          </Line>
          <Line>
            know where to position <Word ref={this.yourself}>yourself</Word> physically<Word ref={this.period}>.</Word> That classroom can
            be
          </Line>
          <Line>your battleground or your playground. And you have to know who</Line>
          <Line>
          ...
          <Line>way. Outside, I say. That’s your territory and when you’re out there</Line>
          <Line>
          ...
          <Line>something people don’t think about. Your territory is like your aura,</Line>
          <Line>
            it goes with you <Word ref={this.everywhere}>everywhere</Word>, in the hallways, on the stairs and,
          </Line>
          <Line>assuredly, in the classroom. Never let them invade your territory.</Line>
          <Line>Never. And remember: teachers who sit or even stand behind their</Line>
          <Line>desks are essentially insecure and should try another line of work.</Line>
          <Line indent>
            <Word>I like</Word>d <Word>the way</Word> he said assuredly, the first time I ever heard it used
          </Line>
          <Line>outside of a Victorian novel. I promised myself that when I became</Line>
          <Line>a teacher I’d use the word, too. It had an important sound to it that</Line>
          <Line>would make people sit up and pay attention.</Line>
          <Line indent>
          ...
          <Line>with everyone before you making notes and if you had any kind of</Line>
          <Line>good looks or personality the girls would be tripping over themselves</Line>
          <Line>
            to see you afterward in your office or <Word>anywhere</Word> else. That’s what I
          </Line>
          <Line>thought at the time.</Line>
        </Page>
      </SVGContainer>
    )
  }
